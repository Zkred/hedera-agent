...
import {
  createCustomPizzaTool,
  getPizzaSizesTool,
  getPizzaToppingsTool,
  getCrustTypesTool,
} from "../tools/pizzaCustomizationTool";
import {
  getLoyaltyPointsTool,
  earnLoyaltyPointsTool,
  redeemLoyaltyPointsTool,
  getLoyaltyTiersTool,
} from "../tools/loyaltyTool";
import {
  validatePromoCodeTool,
  createPromoCodeTool,
  getActivePromoCodesTool,
  deactivatePromoCodeTool,
} from "../tools/promoCodeTool";
import { ChainConfigManager, ChainConfig } from "../utils/chainConfig";

export class PizzaHutAgentService {
  private agent: any;
  ...
  private async initializeAgent(): Promise<void> {
    try {
      ...
      this.agent = createAgent({
        ...
        tools,
        systemPrompt:
          "You are Pizza Hut Delivery Agent, a specialized assistant for pizza orders, customization, loyalty programs, and promotional offers. You can help customers create custom pizzas, manage loyalty points, apply promo codes, and track orders. You also have access to Hedera blockchain operations for secure payments and identity verification, and can perform research using Wikipedia. " +
          "Key capabilities: " +
          "- Create custom pizzas with various sizes, crusts, sauces, cheeses, and toppings " +
          "- Manage customer loyalty points and tier benefits " +
          "- Handle promotional codes and discounts " +
          "- Track pizza orders from preparation to delivery " +
          "- Process secure payments through Hedera blockchain " +
          "- Provide customer support for pizza orders " +
          "All pricing is in HBAR (Hedera cryptocurrency). Always be helpful, friendly, and provide accurate information about pizzas, loyalty programs, and promotions.",
      });

      this.isInitialized = true;
      console.log("Pizza Hut Agent initialized successfully");
      ...
      console.error("Failed to initialize Pizza Hut Agent:", error);
      throw error;
    }
  }
  ...
  private async registerAgentOnChain(): Promise<void> {
    try {
      console.log("Registering Pizza Hut agent on chain...");
      ...
      if (this.agent) {
        ...
        const result = await this.agent.invoke({
          messages: [
            new SystemMessage(
              "You are a helpful assistant that can interact with the Hedera network. You have access to the create_identity tool. Use it to register this Pizza Hut agent on the chain."
            ),
            new HumanMessage(
              `I need to register this Pizza Hut agent on the Hedera network. Please use the create_identity tool with these exact parameters:
              ...
              if (actualContent) {
                ...
                if (toolResult.success) {
                  if (toolResult.agentId) {
                    ...
                    console.log(
                      "Pizza Hut Agent registered with ID:",
                      toolResult.agentId
                    );
                  }
                  ...
                  if (
                    ...
                    console.log(
                      "Pizza Hut Agent is already registered, retrieving existing details..."
                    );
        ...
        console.log("Pizza Hut Agent registration completed successfully");
      } else {
        console.warn("Agent not initialized yet, skipping registration");
      }
    } catch (error) {
      console.error("Error registering Pizza Hut agent on chain:", error);
  ...
  private async retrieveExistingAgentDetails(): Promise<void> {
    try {
      console.log("Retrieving existing Pizza Hut agent details...");
      ...
      if (this.agent && this.chainConfig) {
        ...
        const result = await this.agent.invoke({
          messages: [
            new SystemMessage(
              "You are a helpful assistant that can interact with the Hedera network. You have access to the get_agent_from_service_endpoint tool. Use it to retrieve the existing Pizza Hut agent details."
            ),
            new HumanMessage(
              `Please use the get_agent_from_service_endpoint tool to retrieve the Pizza Hut agent details for service endpoint: "${this.chainConfig.serviceEndpoint}"`
            ),
          ],
        });

        console.log(
          "Pizza Hut Agent details retrieval result:",
        ...
        for (const toolMsg of toolMessages) {
          if (toolMsg.content && typeof toolMsg.content === "string") {
            try {
              ...
              if (actualContent) {
                const toolResult = JSON.parse(actualContent);
                console.log("Retrieved Pizza Hut agent details:", toolResult);
                ...
                if (toolResult.success && toolResult.agent) {
                  ...
                  if (agent.agentId) {
                    ...
                    console.log("Retrieved Pizza Hut Agent ID:", agent.agentId);
                  }

                  if (agent.did) {
                    await this.chainConfigManager.updateDid(agent.did);
                    this.chainConfig!.did = agent.did;
                    console.log("Retrieved Pizza Hut DID:", agent.did);
                  }
                  ...
                  if (agent.publicKey) {
                    ...
                    console.log(
                      "Retrieved Pizza Hut Public Key:",
                      agent.publicKey
                    );
                  }
                }
              ...
              console.error(
                "Could not parse Pizza Hut agent details:",
                parseError
              );
            }
          }
      ...
      console.error(
        "Error retrieving existing Pizza Hut agent details:",
        error
      );
    }
  }
  ...
  async processMessage(message: string): Promise<AgentResponse> {
    ...
    if (isIdentityQuery) {
      if (this.chainConfig) {
        ...
        return {
          content:
            `🍕 **Pizza Hut Agent On-Chain Identity Configuration:**\n\n` +
            `**Agent Details:**\n` +
            ...
            `This Pizza Hut agent identity is registered on the Hedera blockchain and can be verified by other agents.`,
          messages: [],
        };
      } else {
        return {
          content:
            "❌ No identity configuration found. The Pizza Hut agent has not been registered on the blockchain yet.",
          messages: [],
        };
      }
    }
