/**
 * Main server file with A2A (Agent-to-Agent) integration for Pizza Hut Agent
 *
...
function startServer() {
  try {
    console.log("🍕 Starting Pizza Hut Agent with A2A Integration...");
    const server = new SimpleA2AServer();
    server.start(parseInt(PORT.toString()));
  } catch (error) {
    console.error("❌ Failed to start Pizza Hut server:", error);
    process.exit(1);
  }
}
