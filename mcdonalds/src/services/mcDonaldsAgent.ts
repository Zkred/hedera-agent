...
import {
  getComboMealsTool,
  createCustomComboTool,
  getNutritionalInfoTool,
  getAvailableItemsTool,
} from "../tools/comboMealTool";
import {
  getAvailableDriveThruSlotsTool,
  reserveDriveThruSlotTool,
  releaseDriveThruSlotTool,
  updateDriveThruWaitTimeTool,
  getDriveThruStatusTool,
} from "../tools/driveThruTool";
import {
  getMcDeliveryPointsTool,
  earnMcDeliveryPointsTool,
  redeemMcDeliveryRewardTool,
  getAvailableRewardsTool,
  createMcDeliveryRewardTool,
} from "../tools/mcDeliveryRewardsTool";
import { ChainConfigManager, ChainConfig } from "../utils/chainConfig";

export class McDonaldsAgentService {
  private agent: any;
  ...
  private async initializeAgent(): Promise<void> {
    try {
      ...
      this.agent = createAgent({
        ...
        tools,
        systemPrompt:
          "You are McDonald's Delivery Agent, a specialized assistant for fast food orders, combo meals, drive-thru management, and McDelivery rewards. You can help customers create combo meals, reserve drive-thru slots, check nutritional information, and manage rewards. You also have access to Hedera blockchain operations for secure payments and identity verification, and can perform research using Wikipedia. " +
          "Key capabilities: " +
          "- Create and manage McDonald's combo meals with nutritional info " +
          "- Handle drive-thru slot reservations and wait times " +
          "- Manage McDelivery rewards and points system " +
          "- Provide detailed nutritional information for all items " +
          "- Track fast food orders from preparation to delivery " +
          "- Process secure payments through Hedera blockchain " +
          "All pricing is in HBAR (Hedera cryptocurrency). Always be helpful, friendly, and provide accurate information about McDonald's menu, drive-thru, and rewards.",
      });

      this.isInitialized = true;
      console.log("McDonald's Agent initialized successfully");
      ...
      console.error("Failed to initialize McDonald's Agent:", error);
      throw error;
    }
  }
  ...
  private async registerAgentOnChain(): Promise<void> {
    try {
      console.log("Registering McDonald's agent on chain...");
      ...
      if (this.agent) {
        ...
        const result = await this.agent.invoke({
          messages: [
            new SystemMessage(
              "You are a helpful assistant that can interact with the Hedera network. You have access to the create_identity tool. Use it to register this McDonald's agent on the chain."
            ),
            new HumanMessage(
              `I need to register this McDonald's agent on the Hedera network. Please use the create_identity tool with these exact parameters:
              ...
              if (actualContent) {
                ...
                if (toolResult.success) {
                  if (toolResult.agentId) {
                    ...
                    console.log(
                      "McDonald's Agent registered with ID:",
                      toolResult.agentId
                    );
                  }
                  ...
                  if (
                    ...
                    console.log(
                      "McDonald's Agent is already registered, retrieving existing details..."
                    );
        ...
        console.log("McDonald's Agent registration completed successfully");
      } else {
        console.warn("Agent not initialized yet, skipping registration");
      }
    } catch (error) {
      console.error("Error registering McDonald's agent on chain:", error);
  ...
  private async retrieveExistingAgentDetails(): Promise<void> {
    try {
      console.log("Retrieving existing McDonald's agent details...");
      ...
      if (this.agent && this.chainConfig) {
        ...
        const result = await this.agent.invoke({
          messages: [
            new SystemMessage(
              "You are a helpful assistant that can interact with the Hedera network. You have access to the get_agent_from_service_endpoint tool. Use it to retrieve the existing McDonald's agent details."
            ),
            new HumanMessage(
              `Please use the get_agent_from_service_endpoint tool to retrieve the McDonald's agent details for service endpoint: "${this.chainConfig.serviceEndpoint}"`
            ),
          ],
        });

        console.log(
          "McDonald's Agent details retrieval result:",
        ...
        for (const toolMsg of toolMessages) {
          if (toolMsg.content && typeof toolMsg.content === "string") {
            try {
              ...
              if (actualContent) {
                const toolResult = JSON.parse(actualContent);
                console.log("Retrieved McDonald's agent details:", toolResult);
                ...
                if (toolResult.success && toolResult.agent) {
                  ...
                  if (agent.did) {
                    ...
                    console.log("Retrieved McDonald's DID:", agent.did);
                  }
                  ...
                  if (agent.publicKey) {
                    ...
                    console.log(
                      "Retrieved McDonald's Public Key:",
                      agent.publicKey
                    );
                  }
                }
              ...
              console.error(
                "Could not parse McDonald's agent details:",
                parseError
              );
            }
          }
      ...
      console.error(
        "Error retrieving existing McDonald's agent details:",
        error
      );
    }
  }
  ...
  async processMessage(message: string): Promise<AgentResponse> {
    ...
    if (isIdentityQuery) {
      if (this.chainConfig) {
        ...
        return {
          content:
            `🍟 **McDonald's Agent On-Chain Identity Configuration:**\n\n` +
            `**Agent Details:**\n` +
            ...
            `This McDonald's agent identity is registered on the Hedera blockchain and can be verified by other agents.`,
          messages: [],
        };
      } else {
        return {
          content:
            "❌ No identity configuration found. The McDonald's agent has not been registered on the blockchain yet.",
          messages: [],
        };
      }
    }
